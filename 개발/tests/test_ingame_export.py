...
from ingame_mod.scripts.export_ledger_renderer import render_header
...
class IngameExportTests(unittest.TestCase):
    ...
    def test_render_header_keeps_dataset_registry_and_korean_labels(self) -> None:
        header_text, gxt_text = render_header(Path("generated/frontier_ledger_generated.h"))

        self.assertIn("kFrontierLedgerDatasets", header_text)
        self.assertIn('"baseline_en"', header_text)
        self.assertIn("FL_BASELINE_KO_DISPLAY", gxt_text)
