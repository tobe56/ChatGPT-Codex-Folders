...
from .reward_packages import RewardPackage, claim_threshold_packages, last_claimed_package, next_unclaimed_package
...
FARMING_MILESTONE_PACKAGES: dict[str, tuple[RewardPackage, ...]] = {
    "lane.hunter": (
...
def claim_available_farming_milestones(player: PlayerProfile) -> list[str]:
    return claim_threshold_packages(
        player,
        progress_by_id=player.farming_progress,
        packages_by_id=FARMING_MILESTONE_PACKAGES,
        claimed_milestones=player.claimed_farming_milestones,
        label_by_id={lane_id: lane.name for lane_id, lane in FARMING_LANES.items()},
        prefix="Farming milestone",
    )


def _next_package(player: PlayerProfile, lane_id: str) -> RewardPackage | None:
    return next_unclaimed_package(lane_id, FARMING_MILESTONE_PACKAGES.get(lane_id, ()), player.claimed_farming_milestones)


def _last_package(player: PlayerProfile, lane_id: str) -> RewardPackage | None:
    return last_claimed_package(lane_id, FARMING_MILESTONE_PACKAGES.get(lane_id, ()), player.claimed_farming_milestones)


def _lane_next_unlock(progress: int, state: str, lane_name: str, next_package: RewardPackage | None) -> str:
    if state == "Locked":
